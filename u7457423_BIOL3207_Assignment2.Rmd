...
For the significance bias, it seems like a overall probem that cannot by correct by removing some of these studies. To avoid this bias, we can only hope that journals will treat any results equally, regardless of whether they are significant or not. On the other hand, researchers should also ensure these insignificant results are not caused by mistakes. In other words, the insignificant results should also be "reliable".

For the time-lag bias, we can filter the "outstanding" results in year `2009`, `2010` and `2014`.
```{r findy, message = F}
# filter the outstanding studies
head(filter(cInRR, (Year_.online. == 2009 & InRR > 5) | (Year_.online. == 2010 & InRR > 5) | (Year_.online. == 2014 & InRR > 5)))
```

We can see the outstanding values are from study `a2` and `a3`. Let's remove the two study and make a plot again to see the time-lag bias.
```{r filtery, message = F, fig.cap = "Edited relationship between InRR and the year of publication."}
# filter out the two studies
fInRR <- filter(yInRR, (Study != "a2" & Study != "a3" & Study != "a31"))

# plot again
ggplot(fInRR, aes(x = Year_.online., y = InRR)) +
  # make a point plot with different size of different precision
  geom_point(alpha = 0.3, aes(size = 1/sqrt(InRR_V))) +
  # make a regression line with linear model
  geom_smooth(method = "lm", col = "red") +
...
For the time-lag bias, we can filter the "outstanding" results with IF > 9.
```{r findi, message = F}
# filter the outstanding studies
head(filter(cInRR, Pub_year_IF > 9))
```

We can see the outstanding values are from study `a1` and `a3`. `a3` is outstanding twice! Let's remove the two study and make a plot again to see the time-lag bias.
```{r filteri, message = F, fig.width = 20, fig.cap = "Edited relationship between InRR and the imoact factors."}
# filter out the two studies
fInRR <- filter(yInRR, (Study != "a1" & Study != "a3" & Study != "a31"))

# plot again
ggplot(fInRR, aes(x = Pub_year_IF, y = InRR)) +
  # make a point plot with different size of different precision
  geom_point(alpha = 0.3, aes(size = 1/sqrt(InRR_V))) +
  # make a regression line with linear model
  geom_smooth(method = "lm", col = "red") +
...
Overall, `a3` is really a "suspect" study and should be concerned more. Let's read more on Munday *et al.* (2010).

In their study, they investigate the effect of ocean acidification to larval clownfish from the Great Barrier Reef. These characteristic corresponds to two elements that we concluded in the the Figure \@ref(fig:forest3) and Figure \@ref(fig:forest4), `Trop` and `larvae`! Meanwhile, the clownfish lives in coral.Ocean acidification is likely to damage corals, which in turn affects the habitat and behaviour of these fish. This is why do we not see such big effect sizes in other studies. So, the big effect size in Munday *et al.* (2010) is not a universal situation. While citing this article, we should also consider its particularity and the insignificant results of other studies, rather than only relying on impact factor.

Comparing with a meta-analysis by Clement *et al.* (2022), my updated meta-analysis results and their paper all focus on publication including methodological biases, selective publication bias, time-lag bias and citation bias. Because the new added paper from Clark *et al.* (2020) also just showed weak correlation between acidification and fish activity.

Remarkably, in the original data file from Clement *et al.* (2022), the value of column `Average_n` should be the average of the column `ctrl.n` and `oa.n`. But entries of the study `a2` seem not.
...
The authors probably made some mistakes, So, these kind of type error can also be "sampling variance", but we should try our best to avoid it.
